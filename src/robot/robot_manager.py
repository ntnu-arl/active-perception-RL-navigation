...
class RobotManagerIGE(BaseManager):
    ...
    def prepare_for_sim(self, global_tensor_dict):
        ...
        self.global_tensor_dict["robot_actions"] = torch.zeros(
            (self.num_envs, self.robot.num_actions + 2), device=self.device
        )
        ...
        if not self.use_warp:
            ...
            if self.warp_sensor_config is not None:
                ...
                if self.global_tensor_dict["CONST_WARP_MESH_ID_LIST"] is None:
                    ...
                    self.occupancy_map_tensor = torch.zeros(
                            (
                                self.num_envs,
                                self.warp_sensor_config.world_map_grid_size_x,
                                self.warp_sensor_config.world_map_grid_size_y,
                                self.warp_sensor_config.world_map_grid_size_z,
                            ),
                            device=self.device,
                            requires_grad=False,
                        )
                    self.global_tensor_dict["occupancy_map"] = self.occupancy_map_tensor
    ...
    def pre_physics_step(self, actions):
        ...
        self.robot.step(self.actions[:, :4])
