...
class DepthCameraWarpKernels:
    ...
    def draw_optimized_kernel_depth_range(
        ...
        for i in range(101):
            voxel_indices_pointcloud = (ro + distance * rd * free - min)
            free += 0.01
            voxel_indices_pointcloud[0] = wp.clamp(voxel_indices_pointcloud[0] / 12.0 * grid_size_x_occupancy, 0.0, grid_size_x_occupancy)
            voxel_indices_pointcloud[1] = wp.clamp(voxel_indices_pointcloud[1] / 8.0 * grid_size_y_occupancy, 0.0, grid_size_y_occupancy)
            voxel_indices_pointcloud[2] = wp.clamp(voxel_indices_pointcloud[2] / 6.0 * grid_size_z_occupancy, 0.0, grid_size_z_occupancy)
            wp.atomic_max(occupancy_map, env_id, int(voxel_indices_pointcloud[0]), int(voxel_indices_pointcloud[1]), int(voxel_indices_pointcloud[2]), 1.0)
    
        voxel_indices_pointcloud = (ro + dist_pointcloud * rd - min)
        voxel_indices_pointcloud[0] = wp.clamp(voxel_indices_pointcloud[0] / 12.0 * grid_size_x_occupancy, 0.0, grid_size_x_occupancy)
        voxel_indices_pointcloud[1] = wp.clamp(voxel_indices_pointcloud[1] / 8.0 * grid_size_y_occupancy, 0.0, grid_size_y_occupancy)
        voxel_indices_pointcloud[2] = wp.clamp(voxel_indices_pointcloud[2] / 6.0 * grid_size_z_occupancy, 0.0, grid_size_z_occupancy)
        wp.atomic_max(occupancy_map, env_id, int(voxel_indices_pointcloud[0]), int(voxel_indices_pointcloud[1]), int(voxel_indices_pointcloud[2]), 2.0)
